...
from ui.core.utils.dialog_utils import pick_file, pick_save_file, themed_message
...
def _repair_portable_venv(node_dir):
    ...
    host_python_dir = Path(host_python_exe).parent
    ...
    if cfg_path.is_file():
        try:
            ...
            with cfg_path.open("w", encoding="utf-8", newline="") as f:
...
class ImportExportManager:
    ...
    def export_node(self, node_name):
        ...
        try:
            ...
            if node_name not in self.main_window.nodes_data:
                themed_message(self.main_window, t("k_title_error"), f"节点 '{node_name}' 不存在", "error")
                return False
            ...
            if not Path(node_path).is_dir():
                themed_message(self.main_window, t("k_title_error"), f"节点目录不存在: {node_path}", "error")
                return False

            # 使用自绘文件保存对话框
            file_path = pick_save_file(
                self.main_window, t("k_export_node"), default_name=node_name, filter_ext=Packager.BNOS_EXTENSION
            )
            ...
            # 压缩节点目录
            result = Packager.compress_directory(
                node_path, file_path[: -len(Packager.BNOS_EXTENSION)], Packager.BNOS_EXTENSION
            )

            if result:
                themed_message(
                    self.main_window, t("k_title_success"), f"节点 '{node_name}' 已导出到:\n{result}", "success"
                )
                return True
            else:
                themed_message(self.main_window, t("k_title_error"), f"导出节点 '{node_name}' 失败", "error")
                return False

        except Exception as e:
            logger.error(f"导出节点失败: {e}")
            themed_message(self.main_window, t("k_title_error"), f"导出节点失败: {str(e)}", "error")
            return False
    ...
    def export_project(self):
        ...
        try:
            if not self.main_window.current_project_path:
                themed_message(self.main_window, t("k_title_warning"), t("k_project_no_project"), "warning")
                return False
            ...
            # 使用自绘文件保存对话框
            file_path = pick_save_file(
                self.main_window, t("k_export_project"), default_name=project_name, filter_ext=Packager.BNOSC_EXTENSION
            )
            ...
            # 压缩项目目录
            result = Packager.compress_directory(
                project_path, file_path[: -len(Packager.BNOSC_EXTENSION)], Packager.BNOSC_EXTENSION
            )

            if result:
                themed_message(self.main_window, t("k_title_success"), f"项目已导出到:\n{result}", "success")
                return True
            else:
                themed_message(self.main_window, t("k_title_error"), "导出项目失败", "error")
                return False

        except Exception as e:
            logger.error(f"导出项目失败: {e}")
            themed_message(self.main_window, t("k_title_error"), f"导出项目失败: {str(e)}", "error")
            return False
    ...
    def import_node(self):
        ...
        try:
            ...
            # 使用自绘文件选择对话框
            file_path = pick_file(self.main_window, t("k_import_node"), filter_ext=Packager.BNOS_EXTENSION)
            ...
            if not Packager.validate_bnos_package(file_path):
                themed_message(self.main_window, t("k_title_error"), "无效的节点包格式", "error")
                return False
            ...
            try:
                ...
                if not extracted_dir:
                    themed_message(self.main_window, t("k_title_error"), "解压节点包失败", "error")
                    return False

                # 获取节点名称
                node_name = Path(extracted_dir).name

                # 检查目标位置是否已存在
                nodes_dir = Path(self.main_window.current_project_path) / "nodes"
                target_path = nodes_dir / node_name
                ...
                if target_path.exists():
                    ...
                    if reply == QMessageBox.StandardButton.No:
                        ...
                        while target_path.exists():
                            ...
                            target_path = nodes_dir / new_name
                            counter += 1
                        node_name = new_name
            ...
            themed_message(self.main_window, t("k_title_success"), f"节点 '{node_name}' 已导入", "success")
            return True

        except Exception as e:
            logger.error(f"导入节点失败: {e}")
            themed_message(self.main_window, t("k_title_error"), f"导入节点失败: {str(e)}", "error")
            return False
