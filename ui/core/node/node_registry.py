...
class NodeRegistry:
    ...
    def __init__(self, project_path: str):
        ...
        self._registry_path = self._project_path / self.REGISTRY_FILENAME
    ...
    def file_path(self) -> str:
        """注册表文件的完整路径"""
        return str(self._registry_path)
    ...
    def save(self) -> bool:
        ...
        """
        try:
            data = {"nodes": self._nodes, "updated_at": datetime.now().isoformat()}
    ...
    def register_node(self, node_name: str, node_path: str, mount_root: str = None):
        ...
        entry = {
            ...
            "status": "active" if abs_path.is_dir() else "missing",
        }
        if mount_root:
            entry["mount_root"] = str(Path(mount_root).resolve())
        else:
    ...
    def get_node_info(self, node_name: str) -> dict | None:
        """获取单个节点的注册信息
    ...
    def get_mount_root(self, node_name: str) -> str | None:
        """获取节点的挂载根目录路径
    ...
    def get_nodes_by_mount_root(self, mount_root: str) -> dict[str, dict]:
        ...
        Args:
            mount_root: 挂载根目录路径
        """
        mount_root_norm = str(Path(mount_root).resolve())
        return {
            k: v
            for k, v in self._nodes.items()
            if v.get("mount_root") and str(Path(v["mount_root"]).resolve()) == mount_root_norm
        }
