...
class ConnectionInferrer:
    ...
    def infer_all_edges(self) -> list[dict]:
        ...
        for node_name, node_info in self.nodes_data.items():
            ...
            if listen_file and listen_file.strip():
                upstream = self._extract_node_name_from_path(listen_file.strip().replace("\\", "/"))
                if upstream:
            ...
            if isinstance(port_maps, dict):
                for port_name, path in port_maps.items():
                    if not path or not isinstance(path, str):
                        continue
                    upstream = self._extract_node_name_from_path(path.strip().replace("\\", "/"))
                    if upstream:
                        edges.append(
                            {
                                "source": upstream,
    ...
    def compare_with_existing(self, existing_edges: list[dict]) -> dict:
        ...
        new_edges = [e for e in inferred if e["target"] not in existing_map]

        # 缺失：canvas 中有但 config 中没有（可能连线已被删除或手动修改了 config）
        missing_edges = [e for e in existing_edges if e["target"] not in inferred_map]
        ...
        for e in inferred:
            ...
            if target in existing_map and existing_map[target] != e["source"]:
                conflicts.append(
                    {"target": target, "inferred_source": e["source"], "existing_source": existing_map[target]}
                )
    ...
    def get_node_upstream(self, node_name: str) -> str | None:
        """获取单个节点的上游节点名称"""
    ...
    def _extract_node_name_from_path(self, path: str) -> str | None:
        """
    ...
    def diagnose(self) -> str:
        ...
        for node_name, node_info in self.nodes_data.items():
            ...
            if upstream:
                ...
                lines.append("  → 无法解析上游节点（路径可能不在 nodes/ 目录下）")
            else:
                lines.append("  → 无上游连接（孤立节点）")
            lines.append("")
