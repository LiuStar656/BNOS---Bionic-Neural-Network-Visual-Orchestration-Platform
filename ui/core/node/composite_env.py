...
def comp_venv_path(project_path: str, comp_id: str,
    ...
    """
    if display_name:
        return os.path.join(project_path, "nodes", f"{display_name}_venv")
    return os.path.join(project_path, "nodes", f"__comp__{comp_id}_venv")


def get_python_exe(comp_dir: str) -> Optional[str]:
    """获取复合节点 venv 中的 Python 解释器路径。"""
    if os.name == 'nt':
        py = os.path.join(comp_dir, "venv", "Scripts", "python.exe")
    else:
        py = os.path.join(comp_dir, "venv", "bin", "python3")
    return py if os.path.exists(py) else None


def merge_requirements(project_path: str, comp_id: str,
                       display_name: str, node_names: List[str],
                       nodes_data: dict, logger) -> Tuple[bool, str]:
    ...
    for n in node_names:
        ...
        if not node_path:
            node_path = os.path.join(project_path, "nodes", n)
    ...
    # 复合节点独立 venv 路径
    comp_dir = comp_venv_path(project_path, comp_id, display_name)
    ...
    # 安装合并依赖
    merged_path = os.path.join(project_path, ".composite_reqs_tmp.txt")
    try:
        ...
        logger.info(
            ...
            comp_id, len(nodes_with_reqs), len(merged), ", ".join(sorted(merged))
        )
        result = subprocess.run(
            [pip_exe, "install", "-r", merged_path],
...
def remove_comp_env(project_path: str, comp_id: str,
                    display_name: str, logger) -> None:
    """删除复合节点的独立 venv 目录。"""
    import shutil
    comp_dir = comp_venv_path(project_path, comp_id, display_name)
