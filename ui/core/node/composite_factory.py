...
class CompositeNodeFactory:
    ...
    def build(self, node_names: list[str], display_name: str, nodes_data: dict) -> tuple[bool, str, str | None]:
        ...
        if lang_result == "Python":
            builder = PythonCompositeBuilder(self._project_path, self._canvas, self._group_manager)
        else:
            return False, t(TK.COMPOSITE_UNKNOWN_LANGUAGE).format(nodes=f"{lang_result}({'、'.join(node_names)})"), None
...
class PythonCompositeBuilder:
    ...
    def build(self, comp_id: str, display_name: str, node_names: list[str], nodes_data: dict) -> tuple[bool, str]:
        ...
        ok, msg = self._merge_requirements(comp_id, display_name, node_names, nodes_data)
        if not ok:
            return False, msg

        return True, msg

    def generate_orchestrator(self, comp_id: str, node_names: list[str], dag: list[dict], ports: dict) -> str:
    ...
    def _merge_requirements(
        self, comp_id: str, display_name: str, node_names: list[str], nodes_data: dict
    ) -> tuple[bool, str]:
        """Merge child node requirements into composite isolated venv."""
        from ui.core.node.composite_env import merge_requirements as _merge

        return _merge(self._project_path, comp_id, display_name, node_names, nodes_data, logger)
