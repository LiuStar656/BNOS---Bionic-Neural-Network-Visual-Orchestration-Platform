...
from ui.core.node.node_process import detect_running_nodes, start_node_process
...
class JsonNodeStarter:
    ...
    def load_config(self, config_path: str) -> tuple[bool, str, list[dict] | None]:
        """
        加载 JSON 配置文件
    ...
    def _normalize_node_config(self, node_data: dict, config_path: str) -> dict | None:
        """规范化节点配置，处理相对路径"""
        ...
        return {"name": name, "path": path, "config": config, "status": "stopped", "process": None}
    ...
    def _validate_node_config(self, node: dict) -> tuple[bool, str]:
        ...
        # 检查必要文件
        listener_py = node_path / "listener.py"
        if not listener_py.exists():
            return False, f"listener.py 不存在: {listener_py}"

        # 检查配置文件
        has_config = (node_path / "node_config.json").exists()
        if not has_config:
    ...
    def start_nodes_from_config(self, config_path: str) -> tuple[dict[str, bool], str]:
        ...
        # 逐个启动节点
        for node in nodes:
            success, _ = self.start_node(node)
    ...
    def get_loaded_configs(self) -> dict[str, dict]:
        """获取已加载的所有配置"""
