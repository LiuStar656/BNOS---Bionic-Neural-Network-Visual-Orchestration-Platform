...
def _rotate_log(path: str, max_bytes: int = _MAX_LOG_BYTES):
    ...
    try:
        if p.exists() and p.stat().st_size > max_bytes:
            with p.open("rb") as f:
                f.seek(-max_bytes // 2, io.SEEK_END)
                tail = f.read()
            with p.open("wb") as f:
                f.write(b"[... truncated ...]\n")
                f.write(tail)
...
def _open_node_logs(node_path: str):
    ...
    stdout_path = node_path_obj / "node_output.log"
    stderr_path = node_path_obj / "node_error.log"
    # 启动前轮转旧日志
    _rotate_log(str(stdout_path))
    _rotate_log(str(stderr_path))
    stamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    header = f"=== BNOS node started at {stamp} ===\n"
    out_fp = stdout_path.open("w", encoding="utf-8")
    err_fp = stderr_path.open("w", encoding="utf-8")
    out_fp.write(header)
    err_fp.write(header)
    out_fp.flush()
    err_fp.flush()
...
def _is_pid_alive(pid):
    ...
    try:
        if os.name == "nt":
            ...
            # 方法1：使用 tasklist 检查进程是否存在（更可靠）
            try:
                result = subprocess.run(
                    ["tasklist", "/FI", f"PID eq {pid}", "/NH", "/FO", "CSV"], capture_output=True, text=True, timeout=2
                )
                if result.returncode == 0 and result.stdout.strip():
                    # tasklist 返回 CSV 格式："PID","Name",...
                    lines = result.stdout.strip().split("\n")
                    if len(lines) > 1 and str(pid) in lines[0]:
                        return True
            except (subprocess.TimeoutExpired, OSError):
                pass

            # 方法2：使用 OpenProcess 作为备选（但正确处理权限）
...
def _python_exe_for_node(node_path, config=None, node_name=None):
    ...
    # 回退到默认路径
    if os.name == "nt":
        return str(Path(node_path) / "venv" / "Scripts" / "python.exe")
    else:
        return str(Path(node_path) / "venv" / "bin" / "python")
...
def _validate_venv(python_exe):
    ...
    try:
        ...
        result = subprocess.run(
            [python_exe, "--version"], capture_output=True, text=True, timeout=10, creationflags=creationflags
        )
...
def _find_node_processes(node_path):
    ...
    target_exe_norm = str(Path(target_exe)).lower()
    ...
    try:
        if os.name == "nt":
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                exe_path = str(Path(exe_path.strip())).lower()
            ...
            result = subprocess.run(["pgrep", "-f", f"{node_path}"], capture_output=True, text=True, timeout=5)
            for line in result.stdout.strip().split("\n"):
                try:
    ...
    try:
        if os.name == "nt":
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    "终止进程树 PID=%d 失败 (exit=%d): %s", root_pid, result.returncode, result.stderr.strip()
                )
                return False
        else:
...
def _kill_all_node_processes(node_path):
    ...
    for pid in pids:
        try:
            ...
            killed += 1
            logger.info("已清理孤儿进程 PID=%d (节点: %s)", pid, Path(node_path).name)
        except Exception as e:
            logger.warning("清理孤儿进程 PID=%d 失败: %s", pid, e)
    if killed:
        logger.info("共清理 %d 个孤儿进程 (节点: %s)", killed, Path(node_path).name)
    ...
    if unified_path.exists():
        try:
            with unified_path.open(encoding="utf-8") as f:
                cfg = json.load(f)
            logger.debug("使用 node_config.json 配置启动节点")
            return cfg
        except Exception as e:
...
def start_node_process(node_info):
    ...
    try:
        if os.name == "nt":
            ...
            process = subprocess.Popen(
                [python_exe, entry_py], cwd=node_path, start_new_session=True, stdout=out_fp, stderr=err_fp, text=True
            )
        ...
        if exit_code is not None:
            # 读取子进程日志尾部帮助排查
            stderr_tail = _read_tail(str(node_path_obj / "node_error.log"))
            stdout_tail = _read_tail(str(node_path_obj / "node_output.log"))
            error_msg = (
                f"启动失败 (exit={exit_code})\n--- stderr tail ---\n{stderr_tail}\n--- stdout tail ---\n{stdout_tail}"
            )
...
def stop_node_process(node_info, force=False):
    ...
    if not killed:
        ...
        if remaining:
            logger.warning("PID 方式未能终止 %s，使用进程扫描兜底清理 (发现 %d 个进程)", node_name, len(remaining))
            _kill_all_node_processes(node_path)
...
def get_node_log_tail(node_path: str, lines: int = 100) -> dict:
    ...
    node_path_obj = Path(node_path)
    return {
        "stdout": _read_tail(str(node_path_obj / "node_output.log"), lines),
        "stderr": _read_tail(str(node_path_obj / "node_error.log"), lines),
    }
