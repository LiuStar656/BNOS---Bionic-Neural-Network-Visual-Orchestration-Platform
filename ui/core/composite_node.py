...
class CompositeNode:
    ...
    def compress(self, node_names: list, name: str = "") -> Tuple[bool, str, Optional[str]]:
        ...
        for n in node_names:
            ...
            if n not in self._canvas.nodes:
                return False, f"节点 '{n}' 不在画布上", None
        ...
        if cycle_nodes:
            return False, f"以下节点存在循环依赖: {', '.join(sorted(cycle_nodes))}，请检查连线", None
    ...
    def start_inprocess(self, comp_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return True, f"复合节点已启动 (PID={proc.pid})"
        except Exception as e:
    ...
    def start_process_mode(self, comp_id: str) -> Tuple[bool, str]:
        ...
        return True, f"已启动 {len(node_names)} 个独立进程"
