...
from .action_definition import ActionCategory, ActionDefinition
...
def register_view_actions(main_window):
    ...
    ActionRegistry.register(
        ActionDefinition(
            ...
            is_checked_fn=lambda ctx: (
                (getattr(main_window, "node_list_panel", None) is not None and main_window.node_list_panel.isVisible())
                if getattr(main_window, "node_list_panel", None)
                else main_window.app_config.get("panel_visibility", {}).get("node_list_dock", False)
            ),
        )
    )
    ...
    ActionRegistry.register(
        ActionDefinition(
            ...
            is_checked_fn=lambda ctx: (
                ...
                if getattr(main_window, "node_monitor_dock", None)
                else main_window.app_config.get("panel_visibility", {}).get("node_monitor_dock", False)
            ),
        )
    )
    ...
    ActionRegistry.register(
        ActionDefinition(
            ...
            is_checked_fn=lambda ctx: (
                ...
                if getattr(main_window, "resource_monitor", None)
                else main_window.app_config.get("panel_visibility", {}).get("resource_monitor_dock", False)
            ),
        )
    )
    ...
    ActionRegistry.register(
        ActionDefinition(
            ...
            is_checked_fn=lambda ctx: main_window.app_config.get("panel_visibility", {}).get("terminal_dock", False),
        )
    )
