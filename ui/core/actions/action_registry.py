...
sys.path.insert(0, str(_current_dir.parent.parent.parent))

try:
    from ui.core.actions.action_definition import ActionCategory, ActionContext, ActionDefinition
except ImportError:
    spec = importlib.util.spec_from_file_location("action_definition", str(_current_dir / "action_definition.py"))
...
class ActionRegistry:
    ...
    @classmethod
    def get(cls, action_id: str) -> ActionDefinition | None:
        """获取功能定义"""
        return cls._actions.get(action_id)

    @classmethod
    def all(cls, category: ActionCategory | None = None) -> list[ActionDefinition]:
        """获取所有功能定义，可按分类过滤"""
        if category is None:
    ...
    @classmethod
    def execute(cls, action_id: str, context: ActionContext | None = None) -> bool:
        """执行功能"""
    ...
    @classmethod
    def is_enabled(cls, action_id: str, context: ActionContext | None = None) -> bool:
        """检查功能是否可用"""
        action = cls.get(action_id)
        if not action:
            return False
