...
【架构】PollingManager 运行在**后台线程**：
  - 主定时器 + 所有文件 IO（Path.exists/stat/read_text）在后台线程执行
...
class PollingManager(QObject):
    ...
    def __init__(self, parent=None):
        ...
        # 全局日志缓存（最近1000行）
        self._log_cache = {"bnos.log": [], "bnos_error.log": []}
    ...
    def register_task(self, task_name, interval, callback):
        ...
        with self._lock:
            if task_name not in self._tasks:
                self._tasks[task_name] = {"interval": interval, "callback": callback, "enabled": True}
    ...
    def cleanup_node_watchers(self, node_path: str):
        """移除指定节点的所有监控器（线程安全）"""
        with self._lock:
            self._log_watchers = {k: v for k, v in self._log_watchers.items() if k[0] != node_path}
    ...
    def _init_global_watchers(self):
        ...
        for log_file in ["bnos.log", "bnos_error.log"]:
            path = self._logs_dir / log_file
            self._add_global_watcher(path)

        # 监控全局配置文件
        config_files = [
            self._base_dir / "app_config.json",
            self._base_dir / "color_settings.json",
        ]
    ...
    def _poll_node_health(self):
        ...
        from ui.core.node.node_process import check_running_processes

        changes = check_running_processes(nodes_data)
        for name, _code, new_status in changes:
    ...
    def _poll_node_logs(self):
        ...
        for (node_path, log_filename), last_mtime in watchers_snapshot:
            full = Path(node_path) / "logs" / log_filename
            try:
                if not full.exists():
                    continue
    ...
    def _poll_node_output(self):
        ...
        changed = []
        for node_path, (last_mtime, last_content) in watchers_snapshot:
            path = Path(node_path) / "output.json"
            try:
                if not path.exists():
                    continue
    ...
    def _poll_global_config(self):
        ...
        if changed:
            ...
            for path, _, _, should_notify in changed:
                if should_notify:
                    self.global_config_changed.emit(path.name)
    ...
    def get_app_config(self):
        """获取应用配置"""
        config_path = self._base_dir / "app_config.json"
        if config_path.exists():
            try:
    ...
    def get_color_settings(self):
        """获取颜色配置"""
        color_path = self._base_dir / "color_settings.json"
        if color_path.exists():
            try:
                return json.loads(color_path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                return {}
        return {}
