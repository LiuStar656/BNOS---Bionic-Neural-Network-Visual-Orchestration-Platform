...
def save_state(main_window):
    ...
    try:
        ...
        if hasattr(main_window, "_canvas_host") and main_window._canvas_host:
            ...
            canvas_host_state = {"qt_state": canvas_host_state_base64, "area_layouts": canvas_host_area_layouts}
        ...
        logger.info("[SAVE] 保存完成: %d 个 Dock, %d 个区域", len(dock_info), len(area_layouts))
...
def _collect_area_layouts(main_window):
    ...
    for area_enum, area_name in [
        ...
        if area_docks:
            ...
            area_layouts[area_name] = {"orientation": orientation, "docks": area_docks}
...
def _collect_dock_info(main_window):
    ...
    for dock in all_docks:
        ...
        dock_data = {"title": title, "floating": dock.isFloating(), "visible": dock.isVisible(), "area": area_name}

        if dock.isFloating():
            geo = dock.geometry()
            dock_data["geometry"] = {"x": geo.x(), "y": geo.y(), "width": geo.width(), "height": geo.height()}
        else:
            dock_data["size"] = {"width": dock.width(), "height": dock.height()}

        docks_info.append(dock_data)
        logger.debug(
            "[SAVE] 收集 Dock: %s (floating=%s, area=%s, w=%d, h=%d)",
            title,
            dock.isFloating(),
            area_name,
            dock.width(),
...
def _collect_terminal_dock_info(main_window):
    ...
    info = {"visible": term_dock.isVisible(), "floating": term_dock.isFloating()}

    if term_dock.isFloating():
        geo = term_dock.geometry()
        info["geometry"] = {"x": geo.x(), "y": geo.y(), "width": geo.width(), "height": geo.height()}
    else:
        info["size"] = {"width": term_dock.width(), "height": term_dock.height()}
    ...
    for area_name, area_info in area_layouts.items():
        ...
        if area_dock_widgets and area_dock_sizes:
            try:
                ...
                logger.info("[WS] [第%d次] %s区域: %s", pass_num, area_name, area_dock_sizes)
            except Exception as e:
...
def _restore_area_docks_legacy(main_window, area_name, docks_info, pass_num=1):
    ...
    if area_dock_widgets and area_dock_sizes:
        try:
            ...
            logger.info("[WS] [第%d次][旧版] %s: %s", pass_num, area_name, area_dock_sizes)
        except Exception as e:
            logger.warning("[WS] 调整失败: %s", e)
    ...
    for area_name, area_info in area_layouts.items():
        ...
        if area_dock_widgets and area_dock_sizes:
            try:
                ...
                logger.info("[WS] [CanvasHost 第%d次] %s区域: %s", pass_num, area_name, area_dock_sizes)
            except Exception as e:
