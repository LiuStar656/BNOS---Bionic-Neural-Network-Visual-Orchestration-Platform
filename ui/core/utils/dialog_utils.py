...
_STYLE_TREE = (
    "QTreeWidget { background-color: #252526; color: #cccccc; border: 1px solid rgba(255,255,255,10); border-radius: 4px; font-size: 12px; } "
...
class ThemedDialogBase(QDialog):
    ...
    def __init__(self, parent=None, title="", width=400, height=200):
        ...
        self._close_label.setStyleSheet(
            "color: rgba(255,255,255,150); font-size: 14px; padding:0 5px; background:transparent;"
        )
...
def _load_lazy_dir_items(parent_item, full_path, filter_ext=None):
    ...
    try:
        dirs = []
        files = []
        for sub in sorted(full_path.iterdir()):
            name = sub.name
            if sub.is_dir() and not name.startswith(".") and not name.startswith("$"):
        ...
        for name, sub in dirs:
            child = QTreeWidgetItem(parent_item, [name, str(sub)])
            child.setData(0, 1, str(sub))
            try:
                has_subdir = False
                for _sub in sorted(sub.iterdir()):
                    if _sub.is_dir() and not _sub.name.startswith(".") and not _sub.name.startswith("$"):
...
def _create_nav_bar(parent, drives, on_drive_change, on_go_up, show_path_combo=False):
    ...
    drive_combo.setStyleSheet(
        "QComboBox { background: #3c3c3c; color: #ccc; border: 1px solid #555; border-radius: 3px; padding: 2px 4px; font-size: 11px; } QComboBox QAbstractItemView { background: #252526; color: #ccc; }"
    )
    ...
    if show_path_combo:
        ...
        path_combo.setStyleSheet(
            "QComboBox { background: rgba(255,255,255,10); color: #d4d4d4; border: 1px solid rgba(255,255,255,15); border-radius: 4px; padding: 4px 10px; font-size: 12px; } QComboBox QAbstractItemView { background: #252526; color: #ccc; }"
        )
...
def pick_folder(parent, title, start=""):
    ...
    drives = _get_drives()
    sel_path = [str(Path.home())]

    def go_up():
        parent_path = str(Path(sel_path[0]).parent)
    ...
    def on_select(item):
        if item:
            ...
            if path and Path(path).is_dir():
                sel_path[0] = str(Path(path).resolve())
    ...
    dlg.add_button_row([(t("k_cancel"), _STYLE_BTN_GREY, dlg.reject), (t("_k_btn_select"), _STYLE_BTN_OK, dlg.accept)])
    ...
    if dlg.exec() == QDialog.DialogCode.Accepted:
        return str(Path(sel_path[0]).resolve())
    return None
...
def themed_input(parent, title, prompt, default=""):
    ...
    dlg.add_button_row([(t("k_cancel"), _STYLE_BTN_GREY, dlg.reject), (t("k_ok"), _STYLE_BTN_OK, on_accept)])
...
def themed_message(parent, title, text, mode="info"):
    ...
    if mode in ("info", "warning", "error"):
        dlg.add_button_row([(t("k_ok"), _STYLE_BTN_OK, dlg.accept)])
    elif mode == "question":
        dlg.add_button_row(
            [(t("_k_btn_no"), _STYLE_BTN_GREY, dlg.reject), (t("_k_btn_yes"), _STYLE_BTN_OK, dlg.accept)]
        )
    elif mode == "question3":
...
def pick_file(parent, title, filter_ext=None, start=""):
    ...
    nav_bar, drive_combo, up_btn, path_combo = _create_nav_bar(
        parent, drives, lambda idx: load_tree(drive_combo.itemData(idx)), go_up, show_path_combo=True
    )
    lay.addLayout(nav_bar)
    ...
    def on_select(item):
        ...
        if path:
            if Path(path).is_dir():
                ...
                if is_selectable:
                    selected_file[0] = str(Path(path).resolve())
                    file_name_edit.setText(Path(path).name)
                    _preview_file(path)
                else:
                    selected_file[0] = None
                    file_name_edit.clear()
    ...
    def on_confirm():
        if selected_file[0]:
            ...
            selected_file[0] = str((Path(sel_path[0]) / file_name_edit.text().strip()).resolve())
            dlg.accept()
    ...
    dlg.add_button_row([(t("k_cancel"), _STYLE_BTN_GREY, dlg.reject), (t("_k_btn_select"), _STYLE_BTN_OK, on_confirm)])
...
def pick_save_file(parent, title, default_name="", filter_ext=None, start=""):
        ...
        if item:
            ...
            if path:
                if Path(path).is_dir():
                    ...
                    sel_path[0] = str(Path(path).parent.resolve())
                    file_name_edit.setText(Path(path).name)
                current.setText(f"📂 当前: {sel_path[0]}")
    ...
    def on_confirm():
        ...
        if file_name:
            ...
            dlg.selected_path = str((Path(sel_path[0]) / file_name).resolve())
            dlg.accept()

    dlg.add_button_row([(t("k_cancel"), _STYLE_BTN_GREY, dlg.reject), (t("_k_btn_save"), _STYLE_BTN_OK, on_confirm)])
