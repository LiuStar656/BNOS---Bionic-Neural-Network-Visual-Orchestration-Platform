...
from ui.core.node.node_process import (
    resolve_selected_node,
)
from ui.core.project.project_manager import _canvas_call, project_new, project_open, project_refresh
...
from ui.core.system.di import IConfig, container
...
class BNOSMainWindow(
    ...
    def __init__(self):
        ...
        self.panel_manager = PanelManager(self, Path(os.getcwd()) / ".bnos" / "app_config.json")
        ...
        for node_name, node_data in self.nodes_data.items():
            node_control_service.register_node(node_name, node_data.get("path", ""))
        node_control_service.subscribe(self._on_node_service_status)

        # 关闭序列编排器 - 声明式关闭步骤
        self._shutdown_orchestrator = ShutdownOrchestrator()
        self._shutdown_orchestrator.add_step("save_data", lambda: self._shutdown_save_all_data(), depends_on=[])
        self._shutdown_orchestrator.add_step("stop_polling", lambda: polling_manager.stop(), depends_on=["save_data"])
        self._shutdown_orchestrator.add_step(
            "stop_terminal_signals", lambda: self._disconnect_terminal_signals(), depends_on=["save_data"]
        )
        self._shutdown_orchestrator.add_step(
            "stop_terminal_processes", lambda: self._stop_terminal_subprocesses(), depends_on=["stop_terminal_signals"]
        )
        self._shutdown_orchestrator.add_step(
            "stop_ipc", lambda: self._ipc_server.stop() if self._ipc_server else None, depends_on=["stop_polling"]
        )
        self._shutdown_orchestrator.add_step(
    ...
    def _create_toast(
        self, message, toast_type="info", duration=3000, stack_index=0, node_name=None, operation_type=None
    ):
    ...
    def _on_canvas_focused(self, canvas):
        ...
        # 刷新所有面板以适配当前画布
        # 仅在项目路径存在且数据不为空时刷新
        if self.current_project_path and os.path.exists(self.current_project_path) and len(self.nodes_data) > 0:
            self._refresh_panels()
