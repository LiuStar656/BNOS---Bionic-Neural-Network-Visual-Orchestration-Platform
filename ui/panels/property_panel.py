...
class PropertyPanel(QWidget):
        ...
        try:
            ...
            if start_path:
                ...
                if start_path_obj.exists():
                    ...
                    bat_path = start_path_obj / "start.bat"
                    sh_path = start_path_obj / "start.sh"
                    if bat_path.exists():
                        start_script = str(bat_path)
                    elif sh_path.exists():
                        start_script = str(sh_path)
                    else:
                        themed_message(
                            self, t("k_title_error"), t("_k_start_script_missing").format(script=start_script), "error"
                        )
                        return
                ...
                themed_message(
                    self, t("k_title_success"), t("_k_node_started_prop").format(name=self.current_node_name), "info"
                )
    ...
    def stop_node(self):
        ...
        try:
            ...
            themed_message(
                self, t("k_title_success"), t("_k_node_stopped_prop").format(name=self.current_node_name), "info"
            )
        except Exception as e:
