...
class NodeListPanel(FloatingPanel, NodeListOperationsMixin, NodeListDragMixin, NodeListContextMixin):
    ...
    def _init_ui(self):
        ...
        hint_label.setStyleSheet(
            "color: rgba(255, 255, 255, 100); font-size: 10px; font-style: italic; padding: 2px 0;"
        )
    ...
    def update_node_list(self, nodes_data):
        ...
        for group_name, group_info in sorted(groups.items()):
            ...
            group_item.setData(
                ...
                {"type": "group", "name": group_name, "locked": self.group_manager.is_group_locked(group_name)},
            )
    ...
    def batch_delete_nodes(self):
        ...
        def delete_next(index):
            if index >= len(selected_nodes):
                ...
                if self.parent_window:
                    self.parent_window.show_toast(
                        f"已删除 {self._batch_delete_results['success']} 个节点",
                        "success",
                        node_name="batch_operation",
                        operation_type="batch_delete",
                    )
            ...
            self._delete_node_async(
                node_name, lambda ok, err: self._on_batch_delete_node_complete(node_name, ok, err, delete_next, index)
            )
    ...
    def create_group_from_ungrouped(self, ungrouped_nodes):
        ...
        group_name = themed_input_dialog(
            self, t("k_group_create_new"), f"将为 {len(ungrouped_nodes)} 个未分组节点创建新组\n请输入组名称:"
        )
        if not group_name:
            return
        ...
        if self.group_manager.create_group(group_name, color.name()):
            ...
            if self.parent_window:
                self.parent_window.show_toast(
                    f"已创建组 '{group_name}' 并添加 {len(ungrouped_nodes)} 个节点", "success"
                )
