...
```
BNOS/
├── bnos_gui.py                    # Main entry file
├── start_bnos_gui.bat             # Windows startup script
├── requirements_gui.txt           # GUI dependencies
├── app_config.json                # Application-level config (window state, last project path)
│
├── ui/                            # UI module (after modular reorganization)
│   ├── __init__.py                # Unified entry point for convenient imports
...
│   │   └── toast/                 # Toast notification system
│   │       ├── __init__.py
│   │       ├── toast_notification.py  # Toast core class
│   │       └── toast_manager.py   # Toast manager
│   │
│   ├── menu/                      # Menu system
│   │   ├── __init__.py
│   │   └── menu_manager.py        # Menu bar manager
...
│   ├── canvas/                    # Canvas system
│   │   ├── __init__.py
│   │   ├── canvas_view.py         # Canvas view (NodeCanvas)
│   │   └── items/                 # Canvas elements
│   │       ├── __init__.py
│   │       ├── node_item.py       # Node item
│   │       ├── edge_item.py       # Edge item
...
│   │   ├── __init__.py
│   │   ├── node_list_panel.py     # Node list panel
│   │   ├── property_panel.py      # Property panel (config dialog)
│   │   └── node_group_manager.py  # Node group management
│   │
...
│   ├── python_create_node.py      # Python node creation script
│   ├── rust_create_node.py        # Rust node creation script
│   └── README.md
│
├── nodes/                         # Node storage directory (within project)
│   └── [node_name]/               # Single node folder
...
│
└── myenv_new/                     # Python virtual environment
```
...
Adopts modular design, organized by functional domains:

- **core/** - Core base components
  - `app_config.py` - Application configuration management (window state, project path memory)
  - `toast/` - Toast notification system (fully decoupled independent module)

- **menu/** - Menu system
  - `menu_manager.py` - Uniformly manages all menu bar functions

- **canvas/** - Canvas system
  - `canvas_view.py` - Main canvas controller (infinite pan, zoom, box select)
  - `items/` - Canvas element components (nodes, edges, anchors)

- **panels/** - Panel components
  - `node_list_panel.py` - Left node list panel
  - `property_panel.py` - Node configuration dialog
  - `node_group_manager.py` - Node grouping management

- **creators/** - Node creators
  - `node_creator_manager.py` - Multi-language node creation manager

- **docs/** - UI module documentation

#### **tools/** - Utility Scripts

Node creation template generators for various languages:
- `python_create_node.py` - Create Python nodes (with virtual environment)
- `rust_create_node.py` - Create Rust nodes (Cargo project)

#### **nodes/** - Node Storage

Each node is an independent folder containing:
- `config.json` - Node configuration (listening port, output type, etc.)
- `listener.log` - Real-time log
- `venv/` - Independent virtual environment (avoid dependency conflicts)
- Source code files

#### **docs/** - Project Documentation

Detailed technical documentation and update records.
