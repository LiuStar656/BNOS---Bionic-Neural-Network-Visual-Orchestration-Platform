  > ⚠️ **已归档** — 此为 v1 版 README，已于 2026-07-13 被[新版 README](../../README_CN.md) 取代。仅保留作为历史参考。
  ...
  ***
  ...
  📘 **[BNOS 平台技术说明书 →](docs/reference/TECHNICAL_DOCUMENTATION_CN.md)** — 架构设计、核心组件详解、IPC 通信、生命周期管理等完整技术文档（含全项目代码行数统计）
...
> **节点生成器状态**：
> - ✅ **Python 节点生成器**：已完成，可正常使用
> - ⚠️ **Rust 节点生成器**：维护中
> - 🚧 **其他语言生成器**（Node.js、Go、Java、C++、Ruby）：开发中
  ...
  **节点分组管理**
  ...
  - **并发控制**：可配置同时启动的节点数量（默认 2 个），避免批量操作时的资源竞争
  - **优先级调度**：支持设置节点启动优先级，数值越大优先级越高，高优先级节点优先启动
  - **状态反馈**：新增 `queued`（排队中）状态指示器（蓝色 ◎），显示等待启动的节点
  - **错误重试**：启动失败自动重试（默认 3 次），达到上限标记为 `FAILED`
  - **启动间隔控制**：批次间 200ms~500ms 延迟，平滑分配系统资源
  - **队列持久化**：退出时保存队列状态，重启后恢复，保证操作不中断
  - **事件驱动架构**：通过回调事件（`node_enqueued`、`node_starting`、`node_started` 等）解耦队列管理器与 UI
