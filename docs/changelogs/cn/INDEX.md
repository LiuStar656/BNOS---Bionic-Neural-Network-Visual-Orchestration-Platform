...
- **节点选择双源同步修复**：渲染层 `is_selected = _is_custom_selected OR isSelected()` 双源 OR 判定，但 5 处写入口（SelectionManager/SelectedNodesList/CanvasBoxSelect）只清单源导致 `False OR True ≡ True`；修复后所有写入统一双写 `_is_custom_selected + setSelected()`，`on_node_selected` 追加 `scene.clearSelection()` 兜底；单击切换节点旧选中高亮正确清除，Ctrl+单击多选、框选清除、API 级 `box_selected_nodes.clear()` 全链路一致
- **复合节点双击配置页面 + 交互补齐**：`CompositeNodeItem` 完整事件链补齐：`mousePressEvent` 支持普通单击单选、Ctrl+单击多选、右键菜单、连线锚点命中 4 路径；`mouseDoubleClickEvent` 双击打开复合节点配置面板（调用 `canvas.open_node_config(comp_id)`，try/except 异常捕获 + 成功/警告日志）；`NodeDetailPanel.create_for_node` 双 Provider 架构验证（`composite_` 前缀自动走 `CompositeNodeProvider`，3 Tab：概览/集群配置/复合结构，重复「复合配置」Tab 已在前次移除）
- **状态机系统阶段盘点（节点线 + 边线条）**：
...
- **状态机系统**：`ui/core/state/` 独立包，基于 QObject + Signal，4 个 Phase 共 52 项测试全部通过，覆盖节点运行时、复合生命周期、画布模式、边交互
- **节点运行时状态机集成**：NodeRuntimeSM 通过桥接层接入 node_process.py；统一 3 套矛盾状态值；启动/停止/崩溃事件转换
- **复合节点生命周期状态机集成**：CompositeLifecycleSM 接入 composite_node.py；TOCTOU 守卫 + 资源泄漏修复（日志文件句柄清理）
- **启动队列僵尸条目修复**：`_remove_from_queue` 清理终态条目，`enqueue` 防御性检查排除终态，复合节点停止后可正常再次启动
