...
4. **节点启动队列功能实现**：智能 DAG 调度器，支持并发控制、优先级调度、拓扑依赖解析、错误重试、启动间隔控制、队列持久化
5. **节点启动队列与批量停止修复**：修复右键菜单无响应、批量停止只停一个节点、停止后无法重启等 10 个问题
...
[详细内容](./05_节点启动队列功能实现.md)

**核心特性**：

| 功能 | 描述 |
|------|------|
| **并发控制** | 可配置同时启动的节点数量（默认 2 个），避免资源竞争 |
| **优先级调度** | 支持设置节点启动优先级，数值越大优先级越高 |
| **拓扑依赖启动** | 自动解析画布连接关系，按依赖顺序启动（Kahn 算法） |
| **状态反馈** | 新增 `queued`（排队中）、`blocked`（被依赖阻塞）状态 |
| **错误重试** | 支持失败自动重试（默认 3 次），达到上限标记为 `FAILED` |
| **启动间隔控制** | 批次间 200ms~500ms 延迟，避免资源抢占 |
| **队列持久化** | 退出时保存队列状态，重启后恢复 |
| **事件通知** | 通过回调事件（`node_enqueued`、`node_starting`、`node_started` 等）解耦队列管理器与 UI |
