...
- **状态机系统**：`ui/core/state/` 独立包，4 个 Phase 共 52 项测试全部通过，覆盖节点运行时、复合生命周期、画布模式、边交互
- **状态机集成**：NodeRuntimeSM 接入 node_process.py；CompositeLifecycleSM 接入 composite_node.py；TOCTOU 防护 + 资源泄漏修复
- **启动队列修复**：清理终态僵尸条目，复合节点停止后可正常再次启动
