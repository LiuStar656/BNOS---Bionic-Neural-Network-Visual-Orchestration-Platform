...
---
<details open>
<summary><strong>【2026-07-13】V2.0.28 - 复合节点系统完善：右键菜单、拓扑修复、坐标连线、运行态保护、UI重构、启动队列、重命名、多输出端口、DAG状态追踪、README重写</strong></summary>

[查看完整更新](./2026-07-13/README.md) | [01_右键菜单与锚点检测](./2026-07-13/01_复合节点右键菜单优化与输入锚点独占检测.md) | [02_DAG拓扑修复](./2026-07-13/02_复合节点折叠DAG拓扑更新修复.md) | [03_展开坐标修复](./2026-07-13/03_复合节点展开坐标与连线修复.md)

**主要更新：**
- **右键菜单优化**：启动/停止互斥，解耦灰显，展开/折叠运行态双重校验
- **输入锚点独占检测**：一个输入锚点只能接受一条连线，保护数据一致性
- **折叠 DAG 拓扑修复**：`composite.json` edges 实时同步，新拓扑正确生效
- **展开坐标与连线修复**：使用当前位置，端口名称映射，布局过滤，执行顺序调整
- **节点运行态保护**：重命名/删除前检测运行状态，禁止或弹窗确认
- **复合节点 UI 重构**：复用普通节点组件，隐藏冗余元素，添加绿色圆点标记
- **启动队列集成**：复合节点像普通节点一样进入启动队列
- **重命名功能**：节点列表右键菜单支持编辑展示名称
- **多输出端口支持**：锚点管理器优先使用 `output_ports` 配置
- **DAG 状态追踪**：每个子节点的执行状态、错误信息、耗时持久化到 `status.json`
- **README 重写与归档**：~1100 行 → ~220 行，9 章节架构，新增复合节点专题，修正过时引用，旧版归档到 `docs/archived/`

</details>

<details>
<summary><strong>【2026-07-12】V2.0.27 - _port_routing 端口路由重构、代码规范统一化整改与复合节点UI交互第二轮修复</strong></summary>

[查看完整更新](./2026-07-12/README.md) | [02_代码规范](./2026-07-12/02_代码规范统一化整改.md)

**主要更新：**
- **代码规范统一化整改**：全项目 227 文件 Ruff + Pre-commit + EditorConfig 工具链引入；消除 8 个真实运行时 Bug（NameError/ImportError/闭包捕获/死代码）；Logger 4 种写法统一为 `from ui.core.logger import logger`；`print()` 迁移至 `logger.info()`；219 文件新增 `from __future__ import annotations`；`# type: ignore` 8→0 清零；`os.path` → `pathlib.Path` 691→156（-77%）；清理无用 `if/else: pass`、`except Exception: pass` 等死代码；PyLance 0 diagnostics
...
- **复合节点UI交互与连线系统第二轮修复**：右键菜单展开/折叠替代双击；复合节点输出锚点连线；展开态 UI 交互修复（CompositeGroupFrame 形状重写仅覆盖折叠按钮）；折叠后线条残留清理（多复合节点安全）；连线 config.json 双向同步与每次展开/折叠重新对齐；端口识别遵循 config.json；折叠后锚点失效修复（保存→刷新→重新绑定边）；内部节点拖动抖动修复（mousePressEvent 预设锚点 + 每帧更新消除反馈回路）；文件保存 PermissionError 三次重试机制；SelectedNodesList 序列化修复；AnchorItem 参数匹配修复
...
<details>
<summary><strong>【2026-07-08】V2.0.25 - 节点启动机制改进与锚点条件渲染优化</strong></summary>

[查看完整更新](./2026-07-08/README.md) | [01_节点启动机制改进](./2026-07-08/01_节点启动机制改进与自定义入口脚本支持.md) | [02_锚点条件渲染](./2026-07-08/02_输入输出锚点条件渲染优化.md) | [03_代码清理](./2026-07-08/03_参数定义与锚点管理代码清理.md)

**主要更新：**
- 节点启动机制改进：`start.json` 新增 `entry` 字段支持自定义启动程序，默认值为 `main.py`，向后兼容
- 输入输出锚点条件渲染：当 `config.json` 中不存在 `listen_upper_file`/`output_file` 字段时不渲染输入/输出锚点，避免显示多余锚点
- 参数定义兼容性：`ParameterDef` 添加 `description` 字段支持，解析时过滤未知字段防止 TypeError
- 代码清理：移除 `AnchorManager` 中未使用的死代码（`layout_for_rect`、`layout_for_dot` 等），简化代码结构

</details>

<details>
<summary><strong>【2026-07-07】V2.0.24 - 节点名称支持中文命名</strong></summary>

[查看完整更新](./2026-07-07/README.md) | [01_节点名称支持中文命名](./2026-07-07/01_节点名称支持中文命名.md)

**主要更新：**
- 节点名称验证规则变更：从 `^[a-zA-Z0-9_-]+$` 改为 `^[^\\/:*?"<>|]+$`，支持中文及其他 Unicode 字符
- 统一验证架构：`node_list_panel.py` 和 `node_creator_manager.py` 改用 `NodeNameValidator`，确保规则一致
- 更新中英文错误提示文本，反映新的命名规则
...
- 更新日志查看器：菜单栏新增「帮助→查看更新日志」入口，根据语言自动读取中英文 changelogs，QWebEngineView + markdown 库渲染，支持交互式折叠展开
