...
| 文件 | 行数 | 职责 |
|------|------|------|
| `graphic_items.py` | 250 | 图形基类 + 矩形/圆角矩形/多边形/箭头/文本 5 种图形 |
| `draw_layer.py` | 260 | 统一管理图形渲染/选中/拖拽/撤销重做/Alt切换/persist |
| `draw_toolbar.py` | 185 | PS风格 40px 左侧竖式工具栏，VSCode配色，滚轮滚动 |

### 功能清单

**5 种绘图工具**：矩形、圆角矩形、折线多边形（双击闭合）、箭头、文字

**交互设计**：
- 默认模式：鼠标优先操作节点，图形不干扰
- **Alt 键**：切换为图形编辑模式（选中/拖拽/控制点缩放）
- **右键**：直接在图形上右键删除
- 工具栏 **L 键**锁定绘图层、**V 键**显示/隐藏
- 撤销/重做（独立栈，与节点操作互不干扰）

**工具栏设计**：
- 宽度 40px，高度匹配画布，滚轮滚动
- 配色 `#1e1e1e` 统一菜单栏/标题栏
- VSCode 风格选中态：蓝色左竖线高亮，按钮不变形
- 与画布 2px 分割线

**框选**：Alt+左键框选节点，避免与绘图冲突
...
## 🏗️ 进程隔离架构 + 绘图工具计划 (2026-05-22)

### 进程隔离基础设施

四进程架构（主 / 画布 / 面板 / 核心），IPC 通信已就绪：

| 新增文件 | 职责 |
|---------|------|
| `ui/core/ipc.py` | QLocalServer/Client + JSON 编解码，7 个 Action 常量 |
| `ui/core/process_manager.py` | 子进程启动/停止/崩溃检测/自动重启（最多5次） |
| `ui/canvas/canvas_process.py` | 画布子进程入口 |
| `ui/panels/panel_process.py` | 面板子进程入口 |
| `ui/core/core_process.py` | 核心业务后台进程入口 |
| `tests/` | 3 个测试脚本 |

- 通信方案：QLocalSocket + JSON，跨平台无依赖
- 嵌入式模式（默认）正常运行，远程模式基础设施就绪
- 子进程崩溃自动重启，主进程不受影响

### 绘图工具开发计划

PS 风格左侧竖式工具栏 + 绘图层（矩形/多边形/箭头/文字），Alt 键切换编辑模式。✅ 已落地（见上方"绘图工具栏落地"）。
