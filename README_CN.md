...
├── start_bnos_gui.sh              # Linux/Mac 启动脚本
│
├── ui/                            # UI 模块（模块化架构）
│   ├── __init__.py
│   ├── main_window.py            # 主窗口 + Toast 系统
│   ├── canvas_widget.py          # 兼容层（Facade模式，15行）
│   ├── node_list_panel.py        # 带分组的节点列表
│   ├── node_group_manager.py     # 分组管理逻辑
│   ├── node_creator_manager.py   # 节点创建工具管理器
│   ├── property_panel.py         # 配置对话框 + 日志查看器
│   │
│   └── canvas/                   # 🆕 模块化画布架构
│       ├── __init__.py           # 模块导出
│       ├── canvas_view.py        # Core层：NodeCanvas控制器（1763行）
│       ├── CANVAS_SPLIT_REPORT.md # 详细重构文档
│       │
│       └── items/                # Items层：纯UI渲染组件
│           ├── __init__.py       # 图形项导出
│           ├── anchor_item.py    # AnchorItem：输入/输出端口
│           ├── node_item.py      # NodeItem：节点容器
│           └── edge_item.py      # EdgeItem：贝塞尔曲线连线
│
├── tools/                         # 开发工具
│   ├── README.md                 # 工具文档
...
├── color_settings.json            # 节点颜色自定义（.gitignored）
└── requirements_gui.txt           # Python 依赖
```

**架构亮点**：
- ✅ **模块化画布**：拆分为4层（Items、Core、兼容层、导出）
- ✅ **关注点分离**：UI渲染与业务逻辑隔离
- ✅ **向后兼容**：通过Facade模式保持旧导入路径可用
- ✅ **可扩展**：易于添加自定义节点类型和交互
- ✅ **可维护**：每个模块职责单一
